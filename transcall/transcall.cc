...
int main(int argc, char *argv[])
{
...
#ifdef HOST_ID
	if (argc < 3) {
		fprintf(stderr, "usage: transcall dir vm_tag command  \n");
		return 0;
	}
...
#else
	if (argc < 4) {
		fprintf(stderr, "usage: transcall dir dom_id vm_tag command  \n");
		return 0;
	}
	...
	vm_tag_char = argv[argc - 2];
	if(strstr(vm_tag_char,hex) == NULL){
		sprintf(vm_tag_char,"%s%s",hex, argv[argc - 2]);
	}
#endif /*MIYAMA_VM_LIST*/
	vm_tag = strtoul(vm_tag_char, NULL, 16);
	printf("vm_tag %lx \n", vm_tag);
